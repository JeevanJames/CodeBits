...
namespace CodeBits
{
    /// <inheritdoc />
    /// <summary>
    ///     Represents a most-recently-used collection, where the most recently added or accessed item is moved to the top of
    ///     the collection. If the item already exists in the collection at any position, it is also moved to the top.
    /// </summary>
    ...
    public partial class MruCollection<T> : Collection<T>
    {
        ...
        public MruCollection(int capacity) : this(capacity, new MruCollectionOptions<T>())
        {
        }

        public MruCollection(int capacity, MruCollectionOptions<T> options)
        {
            if (capacity <= 0)
            ...
            // Make a copy of the options, so the values cannot be changed by reference later
            _options = new MruCollectionOptions<T> {
            ...
            if (_options.EqualityComparer == null)
            {
                ...
                if (!equatableType.IsAssignableFrom(typeof(T)))
                    throw new ArgumentException(string.Format(
                        "Generic type '{0}' should implement IEquatable<>. Otherwise, use the constructor where an equality comparer can be explicitly specified.",
                        typeof(T).FullName));
        ...
        /// <summary>
        ///     Gets the item at the specified index without triggerring the MRU logic that causes the item to be moved to the top
        ///     of the collection.
        /// </summary>
        ...
        {
            // Access the item directly through the internal collection to prevent the MRU logic from being triggerred.
            return Items[index];
        }

        /// <summary>
        ///     Overrides the indexer's getter to return the item at the specified index and then move it to the top of the
        ///     collection.
        /// </summary>
        ...
        public int Capacity
        {
            ...
            {
                if (value <= 0)
                    throw new ArgumentOutOfRangeException("value", "Capacity should be greater than zero");
                _capacity = value;
                TrimExcess();
            }
        }
        ...
        protected override void InsertItem(int index, T item)
        {
            bool shouldUseMruLogic =
                HasTrigger(MruTriggers.NewItemInserted) || HasTrigger(MruTriggers.ExistingItemInserted);
    ...
    public partial class MruCollection<T>
    {
        /// <inheritdoc />
        /// <summary>
        ///     <see cref="T:System.Collections.Generic.IEqualityComparer`1" /> implementation that wraps an
    ...
    /// <summary>
    ///     Options for initializing an <see cref="MruCollection{T}" /> instance.
    /// </summary>
